...
import { DocumentChartBarIcon, GifIcon, MusicalNoteIcon, DocumentTextIcon, FilmIcon } from '@heroicons/react/24/solid'
...
function NftCard({ nftItem, index, userAddress }: NftCardProps) {
  ...
  const decryptData = async (nft: NFTProps) => {
    ...
    const dm = await decryptUriFile(nft?.image || '', dk);
    nft.image = dm
    setNft(nft)
  ...
  const getTypeContentIcon = (type: string) => {
    switch (type) {
      ...
      case 'video': return (<FilmIcon className="h-60 w-60 brand-color" title='Encrypted video content'> </FilmIcon>)
  ...
  return (<>
    <div key={index} className="border shadow rounded-xl overflow-hidden">
      {decodedFile === 'base64' ? <div className='m-4'>{getTemplateByTypeFile(nft?.image ?? "", handledDescription[0])}</div> :
      ...
      <div id="accordion-collapse" data-accordion="collapse">
        ...
        <div id="accordion-collapse-body-1" className={!expanded ? "hidden": ""}  aria-labelledby="accordion-collapse-heading-1">
          <div className="p-5 font-light border border-b-0 border-gray-200 dark:border-gray-700 dark:bg-gray-900">
            <p className="mb-2 text-xl font-bold text-white"> {nft?.description}</p> 
        </div>
        </div>
        <div className="p-4">
